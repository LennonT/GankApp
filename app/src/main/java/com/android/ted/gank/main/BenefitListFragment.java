...
public class BenefitListFragment extends BaseLoadingFragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    RecyclerView mRecyclerView;  //福利照片的容器
    @Bind(R.id.benefit_swipe_refresh)
    SwipeRefreshLayout mSwipeRefreshLayout;  //下拉刷新容器

    private ArrayList<Image> mAllBenefitImage;   //照片源数据
    private BenefitGoodsItemAdapter mBenefitItemAdapter;        //Adapter
    private UpdateResultReceiver updateResultReceiver = new UpdateResultReceiver();   //通过广播调用Service更新图片
    private Realm mRealm;
    private StaggeredGridLayoutManager mStaggeredGridLayoutManager;   //LayoutManager

    //是否正在更新图片信息
    private boolean bImproveDoing = false;
    private boolean isALlLoad = false;
    private int hasLoadPage = 0;           //当前加载到的页面
    ...
    @Override
    public void onRefresh() {   //SwipeRefreshLayout 刷新的执行
        reloadData();
    }
